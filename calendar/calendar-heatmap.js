...
var calendarHeatmap = {
    ...
    var item_block = calendarHeatmap.items.selectAll('.item-block-year')
      ...
      .attr('fill', function(d) {
        var color = d3.scaleLinear()
          .range(['rgba(0,0,0,0)', calendarHeatmap.color || '#ff4500'])
      ...
      .on('mouseover', function(d) {
        ...
        if (hours > 0) {
          if (days > 0) {
            tooltip_html += '<div><span></span><span>' + (hours === 1 ? '1 alarm' : hours + ' alarms') + '</span></div>';
          } else {
            tooltip_html += '<span>' + (hours === 1 ? '1 alarm' : hours + ' alarms') + '</span></div>';
          }
        }
  ...
  drawYearOverview: function() {
    ...
    calendarHeatmap.items.selectAll('.item-circle')
      ...
      .on('mouseover', function(d) {
        ...
        var tooltip_html = '';
        if(d.total == 0){
          tooltip_html += '<div class="header"><strong>' + 'No fire alarms</strong></div>'
        } else if(d.total == 1){
          tooltip_html += '<div class="header"><strong>' + '1 fire alarm</strong></div>'
        } else {
          tooltip_html += '<div class="header"><strong>' + d.total + ' fire alarms</strong></div>'
        }
        //tooltip_html += '<div class="header"><strong>' + d.total + ' fire alarms</strong></div>';//(d.total ? calendarHeatmap.formatTime(d.total) : 'No time') + ' tracked</strong></div>';
  ...
  formatTime: function(seconds) {
    ...
    var time = '';
    if (hours > 0) {
      time += hours === 1 ? '1 alarm ' : hours + ' alarms ';
    }
    if (minutes > 0) {
